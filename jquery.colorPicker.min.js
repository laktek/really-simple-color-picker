(function($){var selectorOwner,activePalette,cItterate=0,templates={control:$('<div class="colorPicker-picker">&nbsp;</div>'),palette:$('<div id="colorPicker_palette" class="colorPicker-palette" />'),swatch:$('<div class="colorPicker-swatch">&nbsp;</div>'),hexLabel:$('<label for="colorPicker_hex">Hex</label>'),hexField:$('<input type="text" id="colorPicker_hex" />'),personalLabel:$("<div>Cores Personalizadas</div>"),personalColors:$('<div class="colorPicker-swatch-personal">&nbsp;</div>')},transparent=
"transparent",lastColor;$.fn.colorPicker=function(options){var cPiccker=this.each(function(){var element=$(this),opts=$.extend({},$.fn.colorPicker.defaults,options),defaultColor=$.fn.colorPicker.toHex(element.val().length>0?element.val():opts.pickerDefault),newControl=templates.control.clone(),newPalette=templates.palette.clone().attr("id","colorPicker_palette-"+cItterate),newHexLabel=templates.hexLabel.clone(),newHexField=templates.hexField.clone(),personalLabel=templates.personalLabel.clone(),personalColors=
templates.personalColors.clone(),paletteId=newPalette[0].id,swatch;element.data("colorPickerId","colorPicker_palette-"+cItterate);element.data("colorPickerHex","colorPicker_hex-"+cItterate);$.each(opts.colors,function(i){swatch=templates.swatch.clone();if(opts.colors[i]===transparent){swatch.addClass(transparent).text("X");$.fn.colorPicker.bindPalette(newHexField,swatch,transparent)}else{swatch.css("background-color","#"+this);$.fn.colorPicker.bindPalette(newHexField,swatch)}swatch.appendTo(newPalette)});
newHexLabel.attr("for","colorPicker_hex-"+cItterate);newHexField.attr({"id":"colorPicker_hex-"+cItterate,"value":defaultColor});newHexField.bind("keydown",function(event){if(event.keyCode===13){var hexColor=$.fn.colorPicker.toHex($(this).val());$.fn.colorPicker.changeColor(hexColor?hexColor:element.val())}if(event.keyCode===27)$.fn.colorPicker.hidePalette()});newHexField.bind("keyup",function(event){var hexColor=$.fn.colorPicker.toHex($(event.target).val());$.fn.colorPicker.previewColor(hexColor?
hexColor:element.val())});$('<div class="colorPicker_hexWrap" />').append(newHexLabel).appendTo(newPalette);newPalette.find(".colorPicker_hexWrap").append(newHexField);newPalette.find(".colorPicker_hexWrap").append(personalColors);$("body").append(newPalette);newPalette.hide();newControl.css("background-color",defaultColor);newControl.bind("click",function(){if(element.is(":not(:disabled)"))$.fn.colorPicker.togglePalette($("#"+paletteId),$(this))});if(options&&options.onColorChange)newControl.data("onColorChange",
options.onColorChange);else newControl.data("onColorChange",function(){});element.after(newControl);element.bind("change",function(){element.next(".colorPicker-picker -").css("background-color",$.fn.colorPicker.toHex($(this).val()))});element.val(defaultColor).hide();cItterate++});return cPiccker};$.extend(true,$.fn.colorPicker,{toHex:function(color){if(color.match(/[0-9A-F]{6}|[0-9A-F]{3}$/i))return color.charAt(0)==="#"?color:"#"+color;else if(color.match(/^rgb\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*\)$/)){var c=
...
addPersonalColor:function(options){var colors;var limit=8;if(typeof options.colors=="string"){colors=[];colors[0]=options.colors}else colors=options.colors;element=$(options.element);if(element.length<=0)return false;var newControl=templates.control.clone(),personalColors=templates.personalColors.clone(),paletteId=element.data("colorPickerId"),defaultColor=$.fn.colorPicker.toHex(element.val().length>0?element.val():opts.pickerDefault),swatch;var hexFieldStr=element.data("colorPickerHex");var hexField=
$("#"+hexFieldStr);$.each(colors,function(i){swatch=templates.swatch.clone();if(colors[i]===transparent){swatch.addClass(transparent).text("X");$.fn.colorPicker.bindPalette(hexField,swatch,transparent)}else{swatch.css("background-color","#"+this);$.fn.colorPicker.bindPalette(hexField,swatch)}swatch.prependTo($("#"+paletteId+" .colorPicker-swatch-personal"))});hexField.attr("for",hexFieldStr);hexField.attr({"id":hexFieldStr,"value":defaultColor})}});$.fn.colorPicker.defaults={pickerDefault:"FFFFFF",
colors:["000000","993300","333300","000080","333399","333333","800000","FF6600","808000","008000","008080","0000FF","666699","808080","FF0000","FF9900","99CC00","339966","33CCCC","3366FF","800080","999999","FF00FF","FFCC00","FFFF00","00FF00","00FFFF","00CCFF","993366","C0C0C0","FF99CC","FFCC99","FFFF99","CCFFFF","99CCFF","FFFFFF"],addColors:[]}})(jQuery);